...
def menu():
	...
	try:
		...
		os.system('rm -rf login.txt')
	try:
		otw=requests.get('https://graph.facebook.com/me?access_token='+bct));open("login.txt",'w')
		a = json.loads(otw.text)
		nama = a['name']
		id = a['id']
	except KeyError:
...
def pilih_super():
	...
	def main(arg):
		...
		try:
			...
			pass
		try:
			a = requests.get('https://graph.facebook.com/'+user+'/?access_token='+bct)
			b = json.loads(a.text)
...
def clone_dari_daftar_teman():
	...
	for w in kimak['data']:
		...
		id = w['id']
		nama = w['name']
		links = requests.get("https://graph.facebook.com/"+id+"?access_token="+bct)
